...
echo
echo "Готово: $TARGET"
echo "Значок Curty появится в строке меню — наведите курсор на вырез экрана."
echo
echo "Что дальше:"
echo "  • macOS спросит разрешение на управление Spotify или Music — оно нужно для вкладки «Медиа»."
# Вывод сборки уходит в лог и виден только при неудаче, поэтому про подпись
# говорим здесь: иначе человек просто наблюдает, как разрешения слетают после
# каждого обновления, и не знает, что это лечится.
if ! security find-identity -v -p codesigning 2>/dev/null | grep -qF '"Curty Self-Signed"'; then
    echo "  • Разрешения (календарь, плеер) сбросятся при следующем обновлении:"
    echo "    подпись этой сборки привязана к её содержимому. Лечится за две минуты,"
    echo "    раздел «Разрешения и почему они слетают» в README."
fi
