...
Лечится собственным сертификатом. Требование подписи становится «тот же идентификатор, тот же сертификат», и разрешения переживают обновление. Создаётся один раз, минуты на две:

1. «Связка ключей» → меню «Ассистент сертификации» → «Создать сертификат».
2. Имя — **`Curty Self-Signed`**, тип идентификации — «Самоподписанный корневой», тип сертификата — **«Подписывание кода»**. Остальное по умолчанию.
3. Запустите `Scripts/install.sh` ещё раз.
